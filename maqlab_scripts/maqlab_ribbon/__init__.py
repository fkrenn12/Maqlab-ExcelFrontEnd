...
Custom maqlab_ribbon controls can be added using PyXLL without writing and registering
complicated COM controls.

The maqlab_ribbon is defined by an XML document, configured in the pyxll.cfg file.

Calls in the XML document are dispatched to Python functions. This module
demonstrates some of the possible callbacks for a sample maqlab_ribbon.

Most callbacks are passed an IRibbonControl object as a paramter. This is
the maqlab_ribbon control the callback was associated with and has two properties,
...
print("Module 'maqlab_ribbon' __init__.py called.")
try:
    from win32api import MessageBox
except ImportError:
    _log.warning("*** win32api could not be imported.                ***")
    _log.warning("*** Some of the maqlab_ribbon examples will not work.     ***")
...
# All message boxes launched from the maqlab_ribbon are task-modal (meaning that the
